                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="content" id="content" class="my-editor @error('content') is-invalid @enderror" cols="50"
                            rows="30">{{ old('content') ?? $post->content }}</textarea>
                        @error('content')
                            <p class="text-danger mt-1 fs-6">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="upload__box">
                            ...
                            <div id="holder1" class="d-flex justify-content-center gap-3 flex-wrap">
                                @if (old('images'))
                                    ...
                                    @endforeach
                                @else
                                    <img src="{{ $post->images }}" class="img-fluid">
                                @endif
                            </div>
                        </div>
                        @error('images')
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control  @error('title_meta') invalid @enderror" id="title_meta"
                            placeholder="Tiêu đề(SEO)" name="title_meta"
                            value="{{ old('title_meta') ?? $post->title_meta }}" aria-label="Tiêu đề(SEO)" />
                        @error('title_meta')
                            <p class="text-danger mt-1 fs-6">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea name="description_meta" id="description_meta"
                            class="form-control  @error('description_meta') invalid @enderror" placeholder="Mô tả(SEO)" rows="5">{{ old('description_meta') ?? $post->description_meta }}</textarea>
                        ...
                        <select class="form-select @error('category_id') is-invalid @enderror" name="category_id"
                            ...
                            @foreach (getAllCategoriesPost() as $item)
                                <option value="{{ $item->id }}"
                                    @if (old('category_id') == $item->id || $post->category_id == $item->id) @selected(true) @endif>
                                    {{ $item->name }}</option>
                            @endforeach
