...
@section('content')
    ...
    <form class="row" action="{{ route('dashboard.projects.update', $project->id) }}" method="POST"
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control  @error('title') invalid @enderror" id="title"
                            placeholder="Tiêu đề" name="title" value="{{ old('title') ?? $project->title }}"
                            aria-label="Tiêu đề" autofocus />
                        @error('title')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control  @error('description') invalid @enderror"
                            placeholder="Mô tả(SEO)" rows="5">{{ old('description') ?? $project->description }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="content" id="content" class="my-editor @error('content') is-invalid @enderror" cols="50"
                            rows="30">{{ old('content') ?? $project->content }}</textarea>
                        @error('content')
                            <p class="text-danger mt-1 fs-6">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-5">
                        ...
                        <div class="upload__box">
                            ...
                            <input id="thumbnail1" class="form-control" type="text" name="images"
                                value="{{ $project->images }}" hidden multiple>
                ...
                <div class="card-body">
                        ...
                        <select class="form-select @error('category_id') is-invalid @enderror" name="category_id"
                            ...
                            @foreach (getAllCategoriesProject() as $item)
                                <option value="{{ $item->id }}"
                                    @if (old('category_id') == $item->id || $project->category_id == $item->id) @selected(true) @endif>
                                    {{ $item->name }}</option>
                            @endforeach
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('url') invalid @enderror"
                            value="{{ old('url') ?? $project->url }}" name="url" id="url"
